 ...
 */
function hasLorebook(json: unknown): boolean {
 ...
 * Returns card data, spec version, and any extra chunks (for "CharX in PNG" format)
 */
export function extractFromPNG(data: BinaryData): PNGExtractionResult | null {
  // Validate PNG format
  if (!isPNG(data)) {
    return null;
  }
    ...
    for (const chunk of matchingChunks) {
      try {
        ...
        if (!spec && json && typeof json === 'object') {
          const obj = json as Record<string, unknown>;
          let inferredResult: { data: CCv2Data | CCv3Data; spec: 'v2' | 'v3' } | null = null;
